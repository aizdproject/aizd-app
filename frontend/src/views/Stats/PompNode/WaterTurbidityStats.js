...
export default class PotNodeChart extends Component
{
    ...
    async componentDidMount() {
        await axios.get('/api/v1/pomp-nodes')
        .then(res => {
        ...
        await axios.get('/api/v1/node-statistics/water-turbidity')
        .then(res => {
            let turbidity = res.data.water_turbidity;
            let value = 0;
        ...
        return(
            <Card className="text-white bg-warning">
              <CardBody className="pb-0">
                <div className="text-value">{ this.state.Value } NTU</div>
                <div>Water Turbidity</div>
              </CardBody>
