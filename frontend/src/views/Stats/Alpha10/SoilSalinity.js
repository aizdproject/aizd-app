...
export default class PotNodeChart extends Component {
  ...
  async componentDidMount() {
    ...
    await axios.get('/api/v1/alpha/5c7e6839a73fb30004ead950').then(res => {
      ...
      let last_soil_salinity =
        alpha_node.soil_salinity[alpha_node.soil_salinity.length - 1];

      if (alpha_node) {
        alpha_node.soil_salinity.forEach(element => {
          soil_salinities.push(element);
        });
              ...
              datasets: [
                {
                  ...
                  data: soil_salinities
                }
              ]
            },
            Value: last_soil_salinity
          });
        }
      }
    });
      ...
      <Card className='text-white bg-success'>
        <CardBody className='pb-0'>
          <div className='text-value'>
            <CountUp end={this.state.Value} duration={1} decimals={2} /> mg/L
          </div>
