...
export default class PotNodeChart extends Component {
  ...
  async componentDidMount() {
    ...
    await axios.get('/api/v1/alpha/5c7e6839a73fb30004ead950').then(res => {
      ...
      let last_soil_temperature =
        alpha_node.soil_temperature[alpha_node.soil_temperature.length - 1];

      if (alpha_node) {
        alpha_node.soil_temperature.forEach(element => {
          soil_temperatures.push(element);
        });
              ...
              datasets: [
                {
                  ...
                  data: soil_temperatures
                }
              ]
            },
            Value: last_soil_temperature
          });
        }
      }
    });
      ...
      <Card className='text-white bg-success'>
        <CardBody className='pb-0'>
          <div className='text-value'>
            <CountUp end={this.state.Value} duration={1} decimals={2} /> &deg;C
          </div>
